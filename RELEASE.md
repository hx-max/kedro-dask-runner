...
- Added `DaskRunner`, an `AbstractRunner` implementation that executes the pipeline on a Dask cluster.
    - Local threaded cluster by default (`processes=False`), so in-memory datasets keep working; `n_workers`, `threads_per_worker`, `processes` and any other `LocalCluster` argument can be passed to the constructor.
    - An existing `dask.distributed.Client` can be injected with `client=`, and is then reused instead of created and closed by the runner.
    - `retries` is forwarded to Dask, so a failed task can be retried.
    - A cluster whose workers run in another process is validated up front: non-serialisable datasets and nodes are reported before the run, and in-memory datasets that link nodes are rejected with an explicit `ValueError` instead of failing mid-run.
